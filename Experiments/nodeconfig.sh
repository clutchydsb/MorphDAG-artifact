...
do
  ...
  rsync -rtuv root@${machine}:~/MorphDAG/nodefile/* ../Prototype/nodefile/config
done

# generate node files
../Prototype/launch/node_config -n=$1 -le=false
...
do
  ...
  rsync -rtuv ../Prototype/nodefile/ecs/node$j/* root@${machine}:~/MorphDAG/nodefile
  j=$((j+1))
done

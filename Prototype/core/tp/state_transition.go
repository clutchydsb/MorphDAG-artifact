...
func ApplyMessage(db *state.StateDB, msg types.Message) error {
...
func ApplyMessageForNezha(db *state.StateDB, msg types.Message) error {
	return NewStateTransition(msg, db).nezhaCommit()
}

// MimicConcurrentExecution tests speculative execution of Nezha
func MimicConcurrentExecution(db *state.StateDB, msg types.Message) {

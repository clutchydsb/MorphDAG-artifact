...
// Blockchain implements an Occam instance
type Blockchain struct {
	preChainSets map[int]*types.Block
...
func CreateBlockchain(nodeID string, address string, chainNumber int) *Blockchain {
	...
	preChainSets := make(map[int]*types.Block)
...
func (bc *Blockchain) UpdateEdges() {
	...
	for id, blk := range bc.preChainSets {
		if _, ok := connected[id]; !ok {
			remaining = append(remaining, blk.BlockHash)
		}
	}
...
func (bc *Blockchain) Iterator() *Iterator {
	...
	for _, blk := range bc.preChainSets {
		currentHash[string(blk.BlockHash)] = struct{}{}
	}
...
// GetPreviousBlocks obtains all confirmed blocks in the previous epoch
func (bc *Blockchain) GetPreviousBlocks() map[int]*types.Block {
	return bc.preChainSets
}
...
func (bc *Blockchain) MineBlock(transactions []*types.Transaction, con int, stateRoot []byte) *types.Block {
	...
	if newBlock != nil {
		...
		for _, e := range edges {
			...
			newBlock.PrevBlockHash = append(newBlock.PrevBlockHash, blk.BlockHash)
		}
...
func (bc *Blockchain) seekEdges(blockHash []byte, chainID, bitNumber int) []int {
	...
	for {
		...
		for id, blk := range bc.preChainSets {
			id2 := utils.ConvertBinToDec(blk.BlockHash, bitNumber)
			if id2 == chainID {
				edges = append(edges, id)
			}
		}

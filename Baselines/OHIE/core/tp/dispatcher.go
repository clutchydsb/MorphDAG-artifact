...
func (t *Tracker) arrangeWNodes(addrs []string) {
	wNodes := t.graph.CombineNodes("w", t.deps, addrs)
	var col []*RWNode
...
func (tq *tinyQueue) insertNode(node *RWNode) {
	...
	for addr := range rwSet {
		if _, ok := tq.tiny[addr]; !ok {
			newQueue := &queue{
				rSlice: make([]*RWNode, 0, 1000),
				wSlice: make([]*RWNode, 0, 1000),
			}
...
func (tq *tinyQueue) isInConflict(node *RWNode) bool {
	...
	for addr := range rwSet {
		if q, ok := tq.tiny[addr]; ok {
			for _, rw := range rwSet[addr] {
				if strings.Compare(rw.Label, "r") == 0 && len(q.wSlice) > 0 {
					return true
				} else if strings.Compare(rw.Label, "w") == 0 && len(q.rSlice) > 0 {
					return true
				}
			}
		}

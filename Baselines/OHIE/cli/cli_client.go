...
func (cmd *CMDClient) Run() {
	app := &cli.App{
		...
		Commands: []*cli.Command{
			...
			//	Name:      "sendsg",
			//	Aliases:   []string{"ssg"},
			//	Usage:     "Send signal to start servers",
			//	UsageText: "sendsg",
...
//func (cmd *CMDClient) sendSignal() {
//	client, err := rpc.DialHTTP("tcp", "localhost:"+strconv.Itoa(cmd.rpcPort))
//	if err != nil {
//		log.Panic("Connection error: ", err)
//	}
//
//	sRequest := p2p.SendSignalCmd{}
//	sReply := p2p.SendSignalReply{}
//
//	err = client.Call("RPCServer.SendSignal", sRequest, &sReply)
//	if err != nil {
...
func (cmd *CMDClient) SendBatchTxs(cycles int) {
	var loads []int
	// read file storing dynamic workload scale
	bs, err := ioutil.ReadFile(config.LoadScale)
	if err != nil {

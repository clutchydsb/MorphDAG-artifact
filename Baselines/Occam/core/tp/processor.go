...
func (hp *HProcessor) Start(statedb *state.StateDB) {
	go func() {
		for {
			...
			select {
			case job := <-hp.Channel:
				err := hp.ApplyTransaction(job.tx, statedb)
				if err != nil {
					log.Println(err)
				}
...
func (hp *HProcessor) ApplyTransaction(tx *types.Transaction, statedb *state.StateDB) error {
	// set the end time of transaction execution
	tx.SetEnd2()
	msg := tx.AsMessage()
	//err := ApplyMessage(statedb, msg, hp.cache)

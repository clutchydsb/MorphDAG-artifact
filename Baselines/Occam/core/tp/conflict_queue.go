...
type RWNode struct {
	...
	blkID      string
	location   int64
	ops        int // number of accessing accounts
	isHot      bool
	isExecuted bool
}
...
func CreateGraph(deps []string, txs map[string][]*types.Transaction, hotAccounts map[string]struct{}) QueueGraph {
	...
	for addr := range hotGraph {
		for _, node := range hotGraph[addr] {
			...
			for i, rw := range rwSets[addr] {
				// decide whether the read and write operations exist at the same address
				if i > 0 && rw.Label != rwSets[addr][i-1].Label {
					isOpposite = true
					break
				}
	...
	//
	//for _, h := range hotQueues {
	//	for _, t := range h.rSlice {
	//		ss[string(t.tx.ID)] = struct{}{}
	//	}
	//	for _, t := range h.rwSlice {
	//		ss[string(t.tx.ID)] = struct{}{}
	//	}
	//	for _, t := range h.wSlice {
	//		ss[string(t.tx.ID)] = struct{}{}
	//	}
	//}
	//
	//for _, c := range coldGraph {
	//	for _, t := range c {
	//		ss[string(t.tx.ID)] = struct{}{}
	//	}
	//}
...
// CombineNodes combine all nodes in a given set
...
func mapToGraph(deps []string, txs map[string][]*types.Transaction) map[string][]*RWNode {
